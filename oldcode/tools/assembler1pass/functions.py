...
class Assembler:
	...
	#	if relative:
	#		self.labelpass.append({ "label": group.strip("\(\)"), "position": self.current_addr+1, "current_address": self.current_addr })
	#	else:
	...
	def parse_line(self, line):
		...
		if line == '': # Empty line
			...
			if m.group(1) in self.labels:
				...
				self.labels[m.group(1)] = self.current_addr 
	...
	def second_pass(self):
		for item in self.labelpass:
			...
			#if item['current_address'] != None:
			#	b = bytearray(struct.pack('>I', self.labels[item['label']] - item['current_address']))
			#else:
